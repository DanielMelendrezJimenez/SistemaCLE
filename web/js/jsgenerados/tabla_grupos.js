...
function actualiza() {
    if (checkSeleccion) {
        if (data.GrupoId !== -1) {
            var parametros = 'GrupoId=' + data.GrupoId;
            var base64Parametros = encodeURIComponent(btoa(parametros));
            var url = '../../app/moduloGrupos/editarGrupo.do?params=' + base64Parametros;
...
function eliminar() {
    if (tabla.row('.selected').length !== 0) {
        ...
        bootbox.confirm({
            ...
            message: "¿Esta seguro de querer eliminar el siguiente grupo?" +
                    "<br><b>Curso:</b> " + dataRow[0] +
                    "<br><b>Modalidad:</b> " + dataRow[2] +
                    "<br><b>Horario:</b> " + dataRow[3] +
                    "<br>",
            buttons: {
                confirm: {
                    label: 'Sí',
