        ...
        if (result.success) {
            bootbox.confirm({
                ...
                callback: function (result) {
                    if (result === true) {
                        let inpAluGrupo = $("#inpAluGrupo").val();
                        ...
                        let inTipoAlumno = $("#inTipoAlumno").val();
                        let inpTelefono = $("#inpTelefono").val();
                        let inCarrera = $("#inCarrera").val();
                        let inFolio = $("#inFolio").val();
                        let inpReferencia = $("#inpReferencia").val();
                        let inpCantidad = $("#inpCantidad").val();
                        var data = {
                            inpAluGrupo,
                            inNombre,
                            inpAp_pat,
...
document.getElementById('inpCantidad').addEventListener('input', function () {
...
function mensaje(id) {
    switch (id) {
        ...
        case 'inFolio':
            return '<li>El campo folio es obligatorio.</li>';
        case 'inpReferencia':
            return '<li>El campo referencia es obligatorio.</li>';
        case 'inpCantidad':
            return '<li>El campo cantidad es obligatorio.</li>';
    }

}

function formDatostForm(formData) {
    $("#formCompletarInfo")[0].formDatost();
    $.each(formData, function (index, field) {
        var value = $(field).val();
        var id = $(field).attr('id');
...
function completarInfoAlumno(data) {

    $.ajax({
        type: "POST",
        async: true,
        url: "../../app/inscripciones/guardarInformacionAlumno.do",
        dataType: "json",
        data: data,
        beforeSend: function () {
            $("#pageLoader").show();
