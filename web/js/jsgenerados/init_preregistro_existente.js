                    ...
                    if (result === true) {
                        ...
                        if (inTelefono.length !== 10) {
                            ...
                            if (validarcorreo(inCorreo)) {
                                verificarAlumnoRegistrado(data);
...
function formDatostForm(formData) {
    $("#formAddPreRegistroExistente")[0].formDatost();
    $.each(formData, function (index, field) {
        var value = $(field).val();
        var id = $(field).attr('id');
...
function verificarAlumnoRegistrado(data) {

    $.ajax({
        type: "POST",
        async: true,
        url: "../../app/preregistro/guardarAlumnoRegistrado.do",
        dataType: "json",
        data: data,
        beforeSend: function () {
            $("#pageLoader").show();
