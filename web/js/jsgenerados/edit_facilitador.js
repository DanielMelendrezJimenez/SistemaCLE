        ...
        if (result.success) {
            bootbox.confirm({
                ...
                callback: function (result) {
                    if (result === true) {
                        let inpFacilitadorId = $("#inpFacilitadorId").val();
...
function formDatostForm(formData) {
    $("#formEditFacilitador")[0].formDatost();
    $.each(formData, function (index, field) {
        var value = $(field).val();
        var id = $(field).attr('id');
...
function editarFacilitador(data) {

    $.ajax({
        type: "POST",
        async: true,
        url: "../../app/facilitadores/modificarFacilitador.do",
        data: data,
        dataType: 'json',
        beforeSend: function () {
            $('#pageLoader').show();
            ...
            if (data.status === 0) {
                bootbox.dialog({
                    ...
                    message: '<div class="row" style="text-align: justify">' +
                            ...
                            '<p>' + "El facilitador fue editado exitosamente." + '</p>' +
                            '</div>' +
                            '</div>',
                    buttons: {
                        main: {
